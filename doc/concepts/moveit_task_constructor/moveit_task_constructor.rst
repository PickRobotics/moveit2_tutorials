...
| wrapper 예시 - ``Generate Grasp Pose`` stage에 대한 ``Compute IK``입니다. ``Generate Grasp Pose`` stage는 데카르트 포즈 솔루션을 생성합니다. ``Generate Pose`` stage 주위에 ``Compute IK`` stage를 래핑하면 ``Generate Pose`` stage의 데카르트 포즈 솔루션을 사용하여 해당 pose가 되기 위한 IK 솔루션(즉, 로봇의 joint state configuration)을 생성할 수 있습니다.

| MTC가 제공하는 래퍼(wrappers)에 대한 추가 정보는 여기에서 확인하세요 - :ref:`Wrappers`.
...
MTC가 제공하는 모든 stages는 기본 비용 terms을 가지고 있습니다. 궤적을 생성하는 stages는 솔루션으로서 경로 길이(path length)를 비용으로 사용합니다.
