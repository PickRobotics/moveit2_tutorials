...
The Entire Code
---------------
전체 코드는 :codedir:`here in the MoveIt GitHub project<examples/robot_model_and_robot_state/src/robot_model_and_robot_state_tutorial.cpp>`.

.. tutorial-formatter:: ./src/robot_model_and_robot_state_tutorial.cpp

The Launch File
^^^^^^^^^^^^^^^
코드를 실행하려면 다음 두 가지 작업을 수행하는 launch 파일이 필요합니다.:
 * Panda의 URDF 및 SRDF를 파라미터 서버에 로드하고,
 * MoveIt Setup Assistant가 생성한 kinematics_solver 구성을 이 튜토리얼의 클래스를 인스턴스화하는 node의 네임스페이스내에 있는 ROS 파라미터 서버에 놓습니다.
